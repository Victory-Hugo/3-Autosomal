#!/usr/bin/env python3
import os

# 定义脚本模板，其中双大括号用于输出字面量的 { 和 }
template = r"""#!/bin/bash
# 输入文件是bed bim fam文件，通过vcf文件转化而来
# 使用 PLINK 将 VCF 转换为 BED 格式

# 设置基础目录
BASE_DIR="/home/luolintao/Helicopter/Script/分析结果/fineSTRUCTURE"
INPUT_DIR="${{BASE_DIR}}/INPUT/New_Chunk" # 该文件夹内需要准备vcf.gz和索引文件
OUTPUT_DIR="${{BASE_DIR}}/OUTPUT/New_Chunk_{num}/" # 空文件夹
GENETIC_MAP="${{BASE_DIR}}/genetic_map_HP.txt" # 这是HP的遗传图谱文件，按照之前的文献计算的
GENETIC_MAP_FINAL="${{BASE_DIR}}/genetic_map_Finalversion_HP.txt" # 通过genetic_map_HP.txt计算而来
SHAPEIT_BIN="/home/luolintao/Helicopter/Script/分析结果/fineSTRUCTURE/Script/shapeit.v2.904.3.10.0-693.11.6.el7.x86_64/bin/shapeit"
FINESTRUCTURE_SCRIPTS="/home/luolintao/Helicopter/Script/分析结果/fineSTRUCTURE/Script/fineSTRUCTURE"
ChromoPainter_SCRIPTS="/home/luolintao/Helicopter/Script/分析结果/fineSTRUCTURE/Script/ChromoPainterv2"
mkdir -p "$OUTPUT_DIR"
cd "$OUTPUT_DIR"
# 定义输入VCF文件和输出前缀
VCF_FILE="${{INPUT_DIR}}/Chunk_{num}.vcf.gz"
PLINK_PREFIX="$OUTPUT_DIR/Chunk_{num}"
PLINK_HP_PREFIX="$OUTPUT_DIR/Chunk_{num}_HP"

# Step 1: 使用PLINK过滤VCF文件并生成BED格式
# /home/luolintao/Helicopter/Script/分析结果/fineSTRUCTURE/Script/plink_linux_x86_64_20241022/plink --vcf "$VCF_FILE" \
#       --make-bed \
#       --double-id \
#       --allow-extra-chr \
#       --out "$PLINK_PREFIX" \
#       --geno 0.1 \
#       --mind 0.1 

# # 定义PLINK生成的文件
# FILES=("bed" "bim" "fam")
# for EXT in "${{FILES[@]}}"; do
#     FILE="$PLINK_PREFIX.$EXT"
#     cp "$FILE" "$FILE.bak"
# done

# # Step 2: 替换BIM文件中的染色体标识符
# awk 'BEGIN{{OFS="\t"}} {{ $1 = ($1 == "NC_000915.1" ? "1" : $1); print }}' "$PLINK_PREFIX.bim" > "$OUTPUT_DIR/temp.bim" && mv "$OUTPUT_DIR/temp.bim" "$PLINK_PREFIX.bim"

# echo "替换完成！以下文件已更新："
# echo "$PLINK_PREFIX.bim"
# echo "原始文件备份在：$PLINK_PREFIX.bim.bak"

# # Step 3: 使用PLINK选择染色体1并生成新的BED文件
# /home/luolintao/Helicopter/Script/分析结果/fineSTRUCTURE/Script/plink_linux_x86_64_20241022/plink --bfile "$PLINK_PREFIX" \
#       --chr 1 \
#       --make-bed \
#       --alleleACGT \
#       --out "$PLINK_HP_PREFIX"

# # Step 4: 使用Shapeit进行相位处理
# $SHAPEIT_BIN --input-bed "$PLINK_HP_PREFIX.bed" "$PLINK_HP_PREFIX.bim" "$PLINK_HP_PREFIX.fam" \
#              --input-map "$GENETIC_MAP" \
#              --output-max "$OUTPUT_DIR/Chunk_{num}_HP.phased.haps" "$OUTPUT_DIR/Chunk_{num}_HP.phased.sample" \
#              --output-log "$OUTPUT_DIR/Chunk_{num}_HP.log" \
#              --force \
#              --burn 10 \
#              --prune 10 \
#              --main 30 \
#              --thread 8

# # Step 5: 生成ID文件用于ChromoPainter
# #! 特别注意！！这一步生成的ids文件需要进行手动处理，将第二列替换为群体名，第三列替换为1。
# awk 'BEGIN{{FS=" "; OFS="\t"}} {{print $2, $1, $6}}' "$PLINK_HP_PREFIX.fam" | tr -s '\t ' ' ' > "$OUTPUT_DIR/Chunk_{num}_HP.ids"

# # Step 6: 使用Perl脚本转换Shapeit输出为fineSTRUCTURE格式
# echo "使用impute2进行转换，这一步需要耗费很多时间。"
# perl "$FINESTRUCTURE_SCRIPTS/impute2chromopainter.pl" -J \
#      "$OUTPUT_DIR/Chunk_{num}_HP.phased.haps" \
#      "$OUTPUT_DIR/Chunk_{num}_HP.phase"
# dos2unix "$GENETIC_MAP_FINAL"
# echo '保证genetic_map_Finalversion_HP.txt文件是UNIX格式!'

# perl "$FINESTRUCTURE_SCRIPTS/convertrecfile.pl" -M \
#      hap "$OUTPUT_DIR/Chunk_{num}_HP.phase" \
#      "$GENETIC_MAP_FINAL" \
#      "$OUTPUT_DIR/Chunk_{num}_HP.recombfile"

# echo "所有步骤完成！"
# # 删除备份的.bak文件
# rm "$PLINK_PREFIX.bim.bak"
# rm "$PLINK_PREFIX.bed.bak"
# rm "$PLINK_PREFIX.fam.bak"
# echo "备份文件已经删除!"

# # 第三步 计算chromopater所需两参数
# dos2unix "$BASE_DIR/Chunk_{num}_HP.ids"
# echo "保证$BASE_DIR/Chunk_{num}_HP.ids文件是UNIX格式!"
# dos2unix "$BASE_DIR/popDonRec.txt"
# echo "保证$BASE_DIR/popDonRec.txt文件是UNIX格式!"

python3 /home/luolintao/Helicopter/Script/分析结果/fineSTRUCTURE/Script/3_replace_ids.py \
      --inf_csv /home/luolintao/Helicopter/Script/分析结果/fineSTRUCTURE/Anchor.csv \
      /home/luolintao/Helicopter/Script/分析结果/fineSTRUCTURE/OUTPUT/New_Chunk_{num}/Chunk_{num}_HP.ids

# 执行 ChromoPainterv2 命令
"${{ChromoPainter_SCRIPTS}}/ChromoPainterv2" \
    -g "${{OUTPUT_DIR}}/Chunk_{num}_HP.phase" \
    -r "${{OUTPUT_DIR}}/Chunk_{num}_HP.recombfile" \
    -t "${{OUTPUT_DIR}}/Chunk_{num}_HP.ids" \
    -f /home/luolintao/Helicopter/Script/分析结果/fineSTRUCTURE/popDonRec.txt 0 0 -s 0 -n 1089.017 -M 0.038307 \
    -o "${{OUTPUT_DIR}}/Donor_v_Target"
"""

# 循环生成脚本，num 从 3 到 12
for num in range(1, 12):
    filename = f"0_Chromopantainer{num}.sh"
    with open(filename, "w") as f:
        f.write(template.format(num=num))
    # 修改文件权限为可执行（如果需要）
    os.chmod(filename, 0o755)
    print(f"生成 {filename} 成功。")
