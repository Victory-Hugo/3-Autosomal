...
OUTPUT_DIR="${BASE_DIR}/output/Chunk_12/" # 空文件夹
...
cd "$OUTPUT_DIR"
# 定义输入VCF文件和输出前缀
VCF_FILE="${INPUT_DIR}/Chunk_12.vcf.gz"
PLINK_PREFIX="$OUTPUT_DIR/Chunk_12"
PLINK_HP_PREFIX="$OUTPUT_DIR/Chunk_12_HP"
...
$SHAPEIT_BIN --input-bed "$PLINK_HP_PREFIX.bed" "$PLINK_HP_PREFIX.bim" "$PLINK_HP_PREFIX.fam" \
             ...
             --output-max "$OUTPUT_DIR/Chunk_12_HP.phased.haps" "$OUTPUT_DIR/Chunk_12_HP.phased.sample" \
             --output-log "$OUTPUT_DIR/Chunk_12_HP.log" \
             --force \
             --burn 30 \
             --prune 30 \
             --main 30 \
...
awk 'BEGIN{FS=" "; OFS="\t"} {print $2, $1, $6}' "$PLINK_HP_PREFIX.fam" | tr -s '\t ' ' ' > "$OUTPUT_DIR/Chunk_12_HP.ids"
...
perl "$FINESTRUCTURE_SCRIPTS/impute2chromopainter.pl" -J \
     "$OUTPUT_DIR/Chunk_12_HP.phased.haps" \
     "$OUTPUT_DIR/Chunk_12_HP.phase"
dos2unix "$GENETIC_MAP_FINAL"
echo '保证genetic_map_Finalversion_HP.txt文件是UNIX格式!'

perl "$FINESTRUCTURE_SCRIPTS/convertrecfile.pl" -M \
     hap "$OUTPUT_DIR/Chunk_12_HP.phase" \
     "$GENETIC_MAP_FINAL" \
     "$OUTPUT_DIR/Chunk_12_HP.recombfile"
...
# 第三步 
dos2unix "$BASE_DIR/Chunk_12_HP.ids"
echo "保证$BASE_DIR/Chunk_12_HP.ids文件是UNIX格式!"
...
python3 /home/luolintao/S00-Github/3-Autosomal/4-fineSTRCTURE/src/3_replace_ids.py \
      ...
      /home/luolintao/Helicopter/Script/分析结果/fineSTRUCTURE/OUTPUT/Chunk_12/Chunk_12_HP.ids

# 执行 ChromoPainterv2 命令
"${ChromoPainter_SCRIPTS}/ChromoPainterv2" \
    -g "${OUTPUT_DIR}/Chunk_12_HP.phase" \
    -r "${OUTPUT_DIR}/Chunk_12_HP.recombfile" \
    -t "${OUTPUT_DIR}/Chunk_12_HP.ids" \
