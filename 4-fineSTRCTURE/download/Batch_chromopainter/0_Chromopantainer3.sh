...
OUTPUT_DIR="${BASE_DIR}/OUTPUT/New_Chunk_3/" # 空文件夹
...
cd "$OUTPUT_DIR"
# 定义输入VCF文件和输出前缀
VCF_FILE="${INPUT_DIR}/Chunk_3.vcf.gz"
PLINK_PREFIX="$OUTPUT_DIR/Chunk_3"
PLINK_HP_PREFIX="$OUTPUT_DIR/Chunk_3_HP"
...
#              --output-max "$OUTPUT_DIR/Chunk_3_HP.phased.haps" "$OUTPUT_DIR/Chunk_3_HP.phased.sample" \
#              --output-log "$OUTPUT_DIR/Chunk_3_HP.log" \
#              --force \
#              --burn 10 \
#              --prune 10 \
#              --main 30 \
...
# awk 'BEGIN{FS=" "; OFS="\t"} {print $2, $1, $6}' "$PLINK_HP_PREFIX.fam" | tr -s '\t ' ' ' > "$OUTPUT_DIR/Chunk_3_HP.ids"
...
#      "$OUTPUT_DIR/Chunk_3_HP.phased.haps" \
#      "$OUTPUT_DIR/Chunk_3_HP.phase"
# dos2unix "$GENETIC_MAP_FINAL"
# echo '保证genetic_map_Finalversion_HP.txt文件是UNIX格式!'

# perl "$FINESTRUCTURE_SCRIPTS/convertrecfile.pl" -M \
#      hap "$OUTPUT_DIR/Chunk_3_HP.phase" \
#      "$GENETIC_MAP_FINAL" \
#      "$OUTPUT_DIR/Chunk_3_HP.recombfile"
...
# dos2unix "$BASE_DIR/Chunk_3_HP.ids"
# echo "保证$BASE_DIR/Chunk_3_HP.ids文件是UNIX格式!"
...
python3 /home/luolintao/Helicopter/Script/分析结果/fineSTRUCTURE/Script/3_replace_ids.py \
      ...
      /home/luolintao/Helicopter/Script/分析结果/fineSTRUCTURE/OUTPUT/New_Chunk_3/Chunk_3_HP.ids

# 执行 ChromoPainterv2 命令
"${ChromoPainter_SCRIPTS}/ChromoPainterv2" \
    -g "${OUTPUT_DIR}/Chunk_3_HP.phase" \
    -r "${OUTPUT_DIR}/Chunk_3_HP.recombfile" \
    -t "${OUTPUT_DIR}/Chunk_3_HP.ids" \
